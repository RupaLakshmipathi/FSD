...
function homePathFallback() {
  const p = window.location.pathname.replaceAll('\\', '/');
  if (p.includes('/buyer/')) return '../index.html';
  if (p.includes('/seller/')) return '../index.html';
  if (p.includes('/trust/')) return '../index.html';
  return 'index.html';
}
